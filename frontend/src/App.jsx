...
{/* New: Generate Dashboard Button */}
<div className="bg-blue-50 p-6 rounded-lg shadow-inner">
  <h2 className="text-xl font-bold text-blue-800 mb-3">Generate Dashboard</h2>
  <button
    onClick={async () => {
      const firstPdf = allPdfData[0]?.savedname;
      if (!firstPdf) return setError("Please upload a PDF first.");
      try {
        const res = await fetch('http://localhost:5000/dashboard', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ filename: firstPdf })
        });
        const data = await res.json();
        if (data.dashboardUrl) {
          window.open(`http://localhost:5000${data.dashboardUrl}`, '_blank');
        } else {
          setError("Dashboard generation failed.");
        }
      } catch (err) {
        console.error("Dashboard error", err);
        setError("Dashboard request failed.");
      }
    }}
    className="w-full py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white font-semibold rounded-full shadow-md hover:scale-105 transition transform duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
  >
    📈 Generate Dashboard from PDF
  </button>
</div>
